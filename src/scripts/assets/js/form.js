  ...
  function initStepForm(form) {
    ...
    function updateView() {
      questions.forEach(function (question, index) {
        question.classList.toggle("is-active", index + 1 === currentStep);
      });
    ...
    function validateCurrentStep() {
      ...
      if (year || month || day) {
        const isBirthdayValid = year.value && month.value && day.value;

        if (!isBirthdayValid) {
          const birthdayWrap = currentQuestion.querySelector(
            ".p-front__form-birthday",
          );

          showError(birthdayWrap, "生年月日を選択してください");
          isValid = false;
        }
      }
    ...
    function clearErrors(container) {
      container.querySelectorAll(".is-error").forEach(function (el) {
        el.classList.remove("is-error");
      });

      container.querySelectorAll(".p-front__form-error").forEach(function (el) {
        el.remove();
      });
    }
